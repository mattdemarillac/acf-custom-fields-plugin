...
if ( !class_exists(GitLabApi::class, false) ):
	...
	class GitLabApi extends Api {
		...
		public function __construct($repositoryUrl, $accessToken = null, $subgroup = null) {
			...
			$this->repositoryHost = GitLabApi . phpwp_parse_url($repositoryUrl, PHP_URL_HOST) . $port;
			...
			if ( preg_match('@^/?(?P<username>[^/]+?)/(?P<repository>[^/#?&]+?)/?$@', $path, $matches) ) {
				...
				if ( count($segments) > 0 ) {
					$this->repositoryHost = GitLabApi . phptrailingslashit($this->repositoryHost) . implode('/', $segments);
				}
