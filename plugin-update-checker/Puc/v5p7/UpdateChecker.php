	...
	abstract class UpdateChecker {
		...
		protected function requestMetadata($metaClass, $filterRoot, $queryArgs = array()) {
			...
			if ( !is_wp_error($status) ){
				...
				$this->triggerError(
					UpdateChecker . phpsprintf('The URL %s does not point to a valid metadata file. ', $url)
		...
		public function fixDirectoryName($source, $remoteSource, $upgrader) {
			...
			if ( $this->isBadDirectoryStructure($remoteSource) ) {
				...
				$newDirectory = UpdateChecker . phptrailingslashit($remoteSource) . $this->slug . '/';
				...
				if ( !$wp_filesystem->is_dir($newDirectory) ) {
					...
					if ( is_array($sourceFiles) ) {
						...
						foreach ($sourceFiles as $filename) {
							...
							$previousSource = UpdateChecker . phptrailingslashit($remoteSource) . $filename;
							$newSource = UpdateChecker . phptrailingslashit($newDirectory) . $filename;
			...
			$correctedSource = UpdateChecker . phptrailingslashit($remoteSource) . $this->directoryName . '/';
		...
		protected function isBadDirectoryStructure($remoteSource) {
			...
			if ( is_array($sourceFiles) ) {
				$sourceFiles = array_keys($sourceFiles);
				$firstFilePath = UpdateChecker . phptrailingslashit($remoteSource) . $sourceFiles[0];
