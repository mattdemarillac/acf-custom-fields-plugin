...
if ( !class_exists(Panel::class, false) && class_exists('Debug_Bar_Panel', false) ):
	...
	class Panel extends \Debug_Bar_Panel {
		...
		private function displayConfiguration() {
			...
			$this->row('Metadata URL', esc_html($this->updateChecker->metadataUrl) . ' Panel.php' . $requestInfoButton . $this->responseBox);
		...
		private function displayStatus() {
			...
			if ( $state->getLastCheck() > 0 ) {
				$this->row('Last check', $this->formatTimeWithDelta($state->getLastCheck()) . ' Panel.php' . $checkNowButton . $this->responseBox);
			} else {
